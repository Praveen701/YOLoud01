...
<div class="modal fade" id="addcat" tabindex="-1" role="dialog" aria-labelledby="addcat" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered text-left" role="document">
        ...
        <div class="modal-header">
            <h5 class="modal-title" id="addcast">Add Language</h5>
        ...
        <div class="modal-body">
            <div class="form-group">
                <label for="catname">Language Name</label>
                <input type="text" v-model="catname" class="form-control" id="catname" placeholder="Enter Language">
            </div>
        </div>
        <div class="modal-footer">
            <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
            <button type="button" v-on:click="addcatname" class="btn btn-primary">Add Language</button>
        </div>
        </div>
    </div>
</div>
<div class="modal fade" id="editcat" tabindex="-1" role="dialog" aria-labelledby="editcat" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
        <div class="modal-header">
            <h5 class="modal-title" id="editcats">Edit Language</h5>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
            </button>
        </div>
        ...
        <div class="modal-footer">
            <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
            <button type="button" v-on:click="editcatname" class="btn btn-primary">Edit Language</button>
        </div>
        </div>
    </div>
</div>
        ...
        methods:{
            deletemodal(item,index){
                ...
                if(confirm("Are you sure you want to delete?")){
                    axios.post('/admin/settings/language/delete',{
                        id:item.id
                    })
                    .then(response => {
                        if(response.data==1){
                            self.prods.splice(index,1);
                            this.$swal({
                                title: "Success!",
            ...
            editcatname(){
                ...
                var self=this
                if(this.catname!=''){
                    axios.post('/admin/settings/language/edit/'+this.catid,{
                        name:this.catname
                    })
                    .then(response => {
                        if(response.data==1){
            ...
            addcatname(){
                ...
                var self=this
                if(this.catname!=''){
                    axios.post('/admin/settings/language/add',{
                        name:this.catname
                    })
                    .then(response => {
